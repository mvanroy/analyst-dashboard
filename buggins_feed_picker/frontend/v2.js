export default function({ data: config, parentElement, setTriggerValue }) {
  ...
  const makeSelect = (labelText, values, unit, selectedValue, emptyLabel, formatter = null) => {
    ...
    values.forEach((value) => select.appendChild(option(
      String(value),
      formatter ? formatter(value) : `${value} ${unit}`.trim(),
  ...
  const renderBreast = () => {
    ...
    actions.innerHTML = '<button type="button" class="primary" data-action="save">Save feed</button><button type="button" data-action="clear">Clear all</button><button type="button" data-action="cancel">Cancel</button>';
    actions.querySelector('[data-action="save"]').addEventListener("click", () => {
      if (!picker.select.value || time.select.value === "") {
        showError("Choose a valid feed duration and time.");
        return;
      }
      actions.querySelectorAll("button").forEach((button) => { button.disabled = true; });
      send("save", {
        value: Number(picker.select.value),
        minute: Number(time.select.value),
      });
    });
    ...
    app.append(picker.field, time.field, error, actions);
  };
  ...
  const renderBottle = () => {
    ...
    actions.querySelector('[data-action="save"]').addEventListener("click", () => {
      ...
      actions.querySelectorAll("button").forEach((button) => { button.disabled = true; });
      send("save", { feeds, minute: Number(time.select.value) });
    });
    ...
    actions.querySelector('[data-action="cancel"]').addEventListener("click", () => send("cancel"));
    app.append(grid, time.field, error, actions);
  };
